!function(e){
    ...
    Pulsar.registerFunction("readableVariableName", function(e, r, n) {
        ...
        let o = t.join(" ");
        o = o.toLowerCase()
            .replace(/^color/, '')  // Retirer le préfixe "Color"
            .replace(/([a-z])([A-Z])/g, '$1-$2')  // Ajouter un tiret entre les mots
            .replace(/[^a-zA-Z0-9]+/g, '-');  // Remplacer les caractères non alphanumériques par des tirets
    
        if (/^\d/.test(o)) o = "_" + o;  // Si le nom commence par un chiffre, ajouter un underscore
